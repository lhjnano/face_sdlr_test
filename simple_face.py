...
[(288, 67, 490, 380)]
"""
def load_image(jpgfile):
	image = face.load_image(jpgfile)
	return image
...
"""
def face_landmark(image, locations):
...
"""
def face_registerList(face_datas):
	registerList = face.registerList(face_datas)
...
""" 
def face_find(registerList, face_name):
	isExist = False
	for registerPath in registerList :
		if registerPath.find(face_name):
			isExist = True
			break
	return isExist
...
"""
def face_register(face_model_name, face_model_fname, image, face_name):
	face.register(model_name=face_model_name, image=image, label=face_name)
	face.train_face(face_model_fname)
	if os.path.isfile(face_model_fname) == False: 
		return False
...
얼굴 확대 파일 생성 : sr.jpg
"""
def face_super_resolution(subimage, outfile):
	srImage = face.super_resolution(subimage)
	cv2.imwrite(outfile, srImage)
...
"""
def face_super_resolution_prediction(face_model_fname, image, locations, outfile):
	who = face.predict_faces(face_model_fname, image, locations)
